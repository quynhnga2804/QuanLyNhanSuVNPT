import { Table, Button, Descriptions, Upload, Flex, DatePicker, Select, Space, Typography, Modal, Form, Input, message, Image, Row, Col } from 'antd';
...
const EmployeeList = ({ employees, departments, users, fetchEmployees, fetchUsers, employeecontracts }) => {
    ...
    const handleBlock = async (id) => {
        try {
            const response = await put(`/admin/users/${id}`, { Status: 'Inactive' });
    ...
    const handleOpen = async (id) => {
        try {
            const response = await put(`/admin/users/${id}`, { Status: 'Active' });
            if (response.status === 200) {
                message.success('Khóa tài khoản thành công!');
                fetchUsers();
            } else {
                        ...
                        <b>Role:</b> {
                            {
                                ...
                                manager: 'Quản lý',
                                admin: 'Quản trị viên',
                            }[user.Role.toLowerCase()] || 'Không xác định'
                        }
                    </p>
                    <p><b>Status:</b> {user.Status.toLowerCase() === 'active' ? 'Hoạt động' : 'Đã khóa'}</p>
                </div>
            ),
            footer: [
                <Button style={{ float: 'right', marginLeft: '15px' }} key='reset' type='primary' onClick={() => handleResetPassword(user.UserID)}>Cấp lại mật khẩu</Button>,
                <Button style={{ float: 'right' }} key='close' onClick={() => Modal.destroyAll()}>Thoát</Button>,
                user.Status === 'Active' ? (
                    <Button type='primary' danger style={{ float: 'left' }} key='block' onClick={() => handleBlock(user.UserID)}>Khóa</Button>
                ) : (
                    <Button type='primary' style={{ float: 'left', background: 'green' }} key='open' onClick={() => handleOpen(user.UserID)}>Mở</Button>
                )
            ],
        });
    };
    ...
    return (
        <>
            ...
            <Modal className='editfrm' title={<div style={{ textAlign: 'center', width: '100%' }}>Tạo Tài Khoản Người Dùng</div>} open={isCreateModalOpen} onOk={handleCreateAccountSave} onCancel={handleCreateAccountCancel} centered>
                <Form form={createForm} layout='vertical'>
                    ...
                    <Form.Item label='Mật khẩu' name='Password' rules={[{ required: true, message: 'Vui lòng nhập mật khẩu!' }]}>
                        <Input.Password placeholder='Nhập Password' minLength={8} />
                    </Form.Item>
            ...
            {/* Chỉnh sửa */}
            <Modal className='editfrm' width={'70%'} title={<div style={{ textAlign: 'center', width: '100%' }}>Chỉnh Sửa Thông Tin Nhân Sự</div>} open={isEditModalOpen} onOk={() => handleEditSave(editForm, selectedImage)} onCancel={handleEditCancel} centered >
                ...
                <Form form={addForm} layout='vertical'>
                    <Row gutter={24}>
                        ...
                        <Col span={11}>
                            ...
                            <Form.Item label='Email công việc' name='WorkEmail' rules={[{ required: true, type: 'email', message: 'Vui lòng nhập đúng định dạng email!' }]}>
